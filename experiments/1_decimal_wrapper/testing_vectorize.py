...
def run():
    ...
    print(f"> Initialized x_Bjorken type {type(_MASS_OF_PROTON_IN_GEV)}")

    squared_Q_momentum_transfer = np.array([Decimal("1.82") for _ in range(10)])
    print(f"> Initialized squared_Q_momentum_transfer as type {type(_MASS_OF_PROTON_IN_GEV)}")
    ...
    epsilon = np.vectorize(compute_epsilon)(
        x_Bjorken,
